...
import com.example.project.service.RentManager;
...
public class RentManagerTest {
	...
	public void GetAllTypes() {
		...
		assertEquals(startSize, retrievedTypes.size());
	}
	...
	public void UpdateTypeCheck() {
		...
		retrievedType.setName(NAME);
		rentManager.UpdateType(retrievedType);
	}
	...
	public void UpdateEquipmentCheck() {
		...
		int startSize = rentManager.GetEquipmentByType(oldType.getName()).size();
		int startNewNameSize = rentManager.GetEquipmentByType(NEWNAME).size();
		
		Equipment retrievedEquipment = rentManager.GetEquipmentByType(oldType.getName()).get(0);
		...
		assertEquals(startSize - 1, rentManager.GetEquipmentByType(oldType.getName()).size());
		assertEquals(startNewNameSize + 1, rentManager.GetEquipmentByType(NEWNAME).size());
	}
	...
	public void DeleteEquipmentByTypeCheck() {
		...
		int startEquipmentTypeSize = rentManager.GetEquipmentByType(rentManager.GetTypeByName(NEWNAME).getName()).size();

		int startSize = rentManager.GetAllEquipments().size();

		assertEquals(startEquipmentTypeSize, rentManager.GetEquipmentByType(rentManager.GetTypeByName(NEWNAME).getName()).size());
		rentManager.DeleteEquipmentByType(rentManager.GetTypeByName(NEWNAME));
		assertEquals(startEquipmentTypeSize - 1, rentManager.GetEquipmentByType(rentManager.GetTypeByName(NEWNAME).getName()).size());
